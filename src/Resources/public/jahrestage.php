...
class Jahrestage
{
	public function run()
	{
		...
		do {
			...
			if($GLOBALS['TL_CONFIG']['spielerregister_zyklus'])
			{
				// Max. x Newsletter-Empfänger laden, die heute noch keine E-Mail bekommen haben
				$objNewsletter = \Database::getInstance()->prepare("SELECT * FROM tl_newsletter_recipients WHERE active = ? AND pid = ? AND spielerregister_mailTime < ?")
				                                         ->limit($GLOBALS['TL_CONFIG']['spielerregister_zyklus'])
				                                         ->execute(1, $GLOBALS['TL_CONFIG']['spielerregister_newsletter'], $heute);
			}
			else
			{
				// Alle Newsletter-Empfänger laden, die heute noch keine E-Mail bekommen haben
				$objNewsletter = \Database::getInstance()->prepare("SELECT * FROM tl_newsletter_recipients WHERE active = ? AND pid = ? AND spielerregister_mailTime < ?")
				                                         ->execute(1, $GLOBALS['TL_CONFIG']['spielerregister_newsletter'], $heute);
			}
			...
			if($GLOBALS['TL_CONFIG']['spielerregister_wartezeit'])
			{
				// Wartezeit aktivieren
				sleep($GLOBALS['TL_CONFIG']['spielerregister_wartezeit']);
			}
			else
			{
				// Keine Wartezeit definiert, deshalb hier abbrechen
				$weiter = false;
			}

		} while ($weiter == true); // Weitermachen solange weitergemacht werden soll
		echo 'Fertig';
	}
